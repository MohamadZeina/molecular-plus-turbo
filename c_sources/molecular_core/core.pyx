    ...
    with nogil:
        for i in prange(
            ...
            KDTree_rnn_query(
                ...
                &parlist[i],
                parlist[i].loc,
                parlist[i].size * parlist[i].sys.collision_search_multiplier
            )
...
cpdef init(importdata):
    ...
    for i in range(psysnum):
        ...
        psys[i].collision_search_multiplier = importdata[i + 1][6][48] if len(importdata[i + 1][6]) > 48 else 2.0
...
cdef struct ParSys:
    ...
    float collision_search_multiplier
    int links_active
    float link_length
    int link_rellength
    int link_max
